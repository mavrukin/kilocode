...
export const dict = {
  ...
  "settings.sandboxing.hint":
    "Сохраненные изменения сети, назначений и путей для записи применяются перед следующим запуском инструмента в каждом сеансе. При включении песочницы также используются последние настройки.",
